...
public class CodeCoverageRunner {
  ...
  public double run() {
    ...
    String[] mvnCmd = new String[] {
         ...
         */
        "unset MAVEN_CONFIG &&", "./mvnw clean test jacoco:report",  // Run tests
        ...
        profileName != null ? "-D%s=true".formatted(profileName) : "", "--log-file " + outFile,
        // Save output to file
    };

    try {
      String[] cmd = new String[] {"sh", "-c", String.join(" ", mvnCmd)};
      Process p = Runtime.getRuntime().exec(cmd);

      if (p.waitFor() != 0) {
        fail(
            "Erro ao executar teste de cobertura, verifique se os outros testes estão passando, " + "incluindo o linter! Arquivo de log: " + outFile);
      }
      ...
      Map<String, String> sample =
          evaluations.stream().filter(eval -> eval.get("type").equals("LINE"))
